...
def dixiacheng_loop():
    ...
    while True:
        ...
        print('检测次数 %s/10, 休息时间60s' % dixiacount)
...
def jingjichang_loop():
    ...
    if find_pic_inscreen('竞技场挑战'):  # 挑战图标完好，说明没有挑战者，进入 竞技场对战
        ...
        find_pic_andclick('竞技场刷新', pause=5)
    ...
    while True:
        ...
        if find_pic_inscreen('商店图标'):
            break
        click_in_safe()
        print('检测次数 %s/10, 休息时间30s' % dixiacount)
...
def shangdian_loop():
    ...
    for i in range(5):
        find_target_in_fourblock(['两星魔灵','神秘召唤书'])
